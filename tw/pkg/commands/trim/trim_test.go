...
func applyDefaultsTest(packages []string, inputs map[string]configInput) []string {
	...
	for _, pkg := range packages {
		...
		for name, input := range inputs {
			...
			for {
				newPkg := resolvedPkg
				if idx := len(placeholder); idx > 0 {
					newPkg = replaceAll(resolvedPkg, placeholder, input.Default)
				}
				if newPkg == resolvedPkg {
					break
				}
				resolvedPkg = newPkg
			}
		}
		if resolvedPkg != "" && !containsVar(resolvedPkg) {
