...
func (c *Config) processFile(ctx context.Context, filePath string, pipelineResolver *PipelineResolver, out io.Writer) TrimResult {
	...
	for path, packages := range packageLists {
		...
		redundant := c.findRedundantPackages(packages, depResolver, pipelineResolver, pipelineUses, path, out)
		if len(redundant) == 0 {
			continue
		}
...
func (c *Config) findRedundantPackages(
	...
	for _, pkgA := range packages {
		...
		if depResolver != nil {
			for _, pkgB := range packages {
				if pkgA == pkgB {
					continue
				}
				if depResolver.IsTransitiveDep(pkgA, pkgB) {
					redundant = append(redundant, RedundantPkg{
						Package:    pkgA,
						ProvidedBy: pkgB,
