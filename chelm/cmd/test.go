...
var extractors = map[string]chelm.Extractor{
	"structured": chelm.StructuredExtractor{},
...
Exit code is non-zero if any test case fails.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, tc := range meta.Test.Cases {
			...
			// Build ignore set for faster lookup
			// Normalize ignore entries so users can write short names like "busybox:1.36"
			// instead of fully-qualified "index.docker.io/library/busybox:1.36"
			ignoreSet := make(map[string]bool)
			for _, ig := range meta.Test.Ignore {
				if ref, err := name.ParseReference(ig); err == nil {
					ignoreSet[ref.Name()] = true
				} else {
					// Keep original for non-parseable entries (e.g., "auto")
					ignoreSet[ig] = true
				}
			}

			// Build set of valid image IDs for this test case
			validImageIDs := make(map[string]bool)
			for _, id := range tc.Images {
				validImageIDs[id] = true
			}

			// Validate each extracted image is fully parameterized with test markers
			for _, ref := range extraction.All {
				imgStr := ref.FullRef
				caseOut.Images = append(caseOut.Images, imgStr)
